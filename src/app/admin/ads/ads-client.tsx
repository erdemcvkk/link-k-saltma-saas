...
import { ArrowLeft, Save, Code, Image as ImageIcon, AlertCircle, Sparkles, ExternalLink, ShieldCheck, CheckCircle2, QrCode, Layout } from "lucide-react";
...
export default function AdsClient({ adminUserId, initialSettings }: AdsClientProps) {
  ...
  // QR page ad states
  const [qrAdScript, setQrAdScript] = useState(initialSettings.qrAdScript);
  const [qrCustomImageUrl, setQrCustomImageUrl] = useState(initialSettings.qrCustomImageUrl);
  const [qrCustomTargetUrl, setQrCustomTargetUrl] = useState(initialSettings.qrCustomTargetUrl);
  const [isQrActive, setIsQrActive] = useState(initialSettings.isQrActive);

  // Active Tab: "profiles" | "qr"
  const [activeTab, setActiveTab] = useState<"profiles" | "qr">("profiles");
    ...
    startTransition(async () => {
      try {
        const res = await saveSystemSettings(
          ...
          isActive,
          qrAdScript || null,
          qrCustomImageUrl || null,
          qrCustomTargetUrl || null,
          isQrActive
        );
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-3 bg-zinc-950 border border-zinc-800/80 p-3 rounded-2xl shrink-0">
            <div className="space-y-0.5 text-left">
              <span className="text-xs font-bold block text-white">
                {activeTab === "profiles" ? "Profiller Reklam Yayını" : "QR Sayfası Reklam Yayını"}
              </span>
              <span className="text-[10px] text-zinc-500 block">
                {activeTab === "profiles" ? "Tüm ücretsiz profilleri etkiler" : "QR oluşturucu sayfasını etkiler"}
              </span>
            </div>
            <button
              type="button"
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                (activeTab === "profiles" ? isActive : isQrActive) ? "bg-teal-500" : "bg-zinc-800"
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                  (activeTab === "profiles" ? isActive : isQrActive) ? "translate-x-5" : "translate-x-0"
                }`}
              />
            </button>
          </div>
        ...
        {/* Tab Seçimi */}
        <div className="flex bg-zinc-950 p-1.5 rounded-2xl border border-zinc-900 max-w-md">
          <button
            type="button"
        ...
        <form onSubmit={handleSave} className="space-y-6">
          {activeTab === "profiles" ? (
            <div key="profiles-ads" className="space-y-6 animate-in fade-in duration-200">
              ...
              <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 md:p-6 space-y-4 shadow-xl relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-white">Google AdSense / Harici Script Kodu (Profiller)</h3>
                    <p className="text-[10px] text-zinc-500">Kullanıcı profillerine entegre etmek istediğiniz reklam JS kodunu girin.</p>
                  </div>
                </div>
              ...
              <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 md:p-6 space-y-4 shadow-xl relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-white">Özel Görsel Banner (Alternatif - Profiller)</h3>
                    <p className="text-[10px] text-zinc-500">Profiller için görsel reklam (genellikle yatay banner biçiminde).</p>
                  </div>
                </div>
            ...
            <div key="qr-ads" className="space-y-6 animate-in fade-in duration-200">
              {/* Bölüm 1: Script Entegrasyonu (AdSense - QR) */}
              <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 md:p-6 space-y-4 shadow-xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-24 h-24 bg-teal-500/5 rounded-full blur-2xl pointer-events-none" />
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-2xl bg-teal-950/30 text-teal-400 flex items-center justify-center border border-teal-500/20">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-white">Google AdSense / Harici Script Kodu (QR Tasarlayıcı)</h3>
                    <p className="text-[10px] text-zinc-500">QR oluşturucu sayfasına entegre etmek istediğiniz reklam JS kodunu girin.</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    value={qrAdScript}
                    onChange={(e) => setQrAdScript(e.target.value)}
                    placeholder="<!-- Google AdSense Code -->&#10;<script async src='https://pagead2.googlesyndication.com/...'></script>"
                    rows={6}
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-4 text-xs font-mono text-emerald-400 focus:outline-none focus:border-teal-500 transition-colors focus:ring-1 focus:ring-teal-500/20 placeholder-zinc-700"
                  />
                  <div className="flex items-start gap-2 text-[10px] text-zinc-500 bg-zinc-900/40 p-3 rounded-xl border border-zinc-800/20">
                    <AlertCircle className="h-4 w-4 text-zinc-400 shrink-0" />
                    <span>
                      <strong>Not:</strong> Bu alana eklenen script kodu, QR Tasarlayıcı (/qr-olusturucu) sayfasında dikey ve yatay reklam alanlarında render edilecektir.
                    </span>
                  </div>
                </div>
              </div>

              {/* Bölüm 2: Özel Banner (Yedek Plan - QR) */}
              <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 md:p-6 space-y-4 shadow-xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/5 rounded-full blur-2xl pointer-events-none" />
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-2xl bg-purple-950/30 text-purple-400 flex items-center justify-center border border-purple-500/20">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-white">Özel Görsel Banner (Alternatif - QR Tasarlayıcı)</h3>
                    <p className="text-[10px] text-zinc-500">QR Tasarlayıcı sayfası için görsel reklam. (Masaüstünde dikey gökdelen olarak uzatılır, mobilde yatay kırpılır).</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="url"
                      value={qrCustomImageUrl}
                      onChange={(e) => setQrCustomImageUrl(e.target.value)}
                      placeholder="https://example.com/qr-banner.jpg"
                      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-purple-500 transition-colors"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="url"
                      value={qrCustomTargetUrl}
                      onChange={(e) => setQrCustomTargetUrl(e.target.value)}
                      placeholder="https://hedefsite.com/qr-kampanya"
                      className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-purple-500 transition-colors"
                    />
                  </div>
                </div>
                ...
                {/* Önizleme Alanı */}
                {qrCustomImageUrl && (
                  <div className="mt-4 p-4 bg-zinc-900/30 rounded-2xl border border-zinc-800/40 space-y-4 text-left">
                    <span className="text-[10px] font-semibold text-zinc-400 uppercase tracking-wider block">Görsel Önizlemeleri</span>
                    
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                      {/* Dikey Önizleme */}
                      <div className="space-y-1.5 shrink-0">
                        <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Masaüstü Dikey (Skyscraper)</span>
                        <div className="relative rounded-xl overflow-hidden border border-zinc-800 w-[120px] h-[300px] bg-black">
                          <img 
                            src={qrCustomImageUrl} 
                            alt="QR Dikey Önizleme" 
                            className="w-full h-full object-cover"
                            onError={(e) => {
                              (e.target as HTMLElement).style.display = 'none';
                            }}
                          />
                          <div className="absolute top-1 right-1 bg-black/60 text-[6px] text-zinc-400 px-1 py-0.5 rounded border border-white/5">
                            REKLAM
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-1.5 flex-1 w-full">
                        <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Mobil Yatay Banner</span>
                        <div className="relative rounded-xl overflow-hidden border border-zinc-800 aspect-[5/1] bg-black max-w-md w-full">
                          <img 
                            src={qrCustomImageUrl} 
                            alt="QR Yatay Önizleme" 
                            className="w-full h-full object-cover"
                            onError={(e) => {
                              (e.target as HTMLElement).style.display = 'none';
                            }}
