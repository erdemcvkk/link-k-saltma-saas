...
interface YardimClientProps {
  faqs?: Array<{ question: string; answer: string }>;
}

export default function YardimClient({ faqs = [] }: YardimClientProps) {
  const [openIndex, setOpenIndex] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");

  const dynamicCategory: FAQCategory | null = faqs.length > 0 ? {
    title: "Genel Sorular",
    icon: HelpCircle,
    items: faqs
  } : null;

  const displayData = dynamicCategory ? [dynamicCategory, ...FAQ_DATA] : FAQ_DATA;
  ...
  const filteredCategories = displayData.map((category, catIdx) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 pb-16">
      {/* Hero Title Area inside Page Main */}
