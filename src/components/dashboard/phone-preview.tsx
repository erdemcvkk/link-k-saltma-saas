...
export default function PhonePreview({ mode, data, label, activeAddonId }: PhonePreviewProps) {
  ...
  const renderPluginPreview = () => {
    ...
    if (["PREMIUM_VIDEO", "SPOTIFY_CLASSIC", "VINYL_RETRO", "GLASS_AUDIO", "NEON_CYBERPUNK", "MUSIC_PODCAST", "PORTFOLIO_GALLERY", "COUNTDOWN_LAUNCH", "RETRO_CASSETTE", "AUDIO_PLAYER", "TRAVEL_STOREFRONT", "ELITE_TRAVEL", "PREMIUM_PROFILE"].includes(type)) {
      ...
      const displayTitle = parsedConfig.title || (type === "AUDIO_PLAYER" ? "Premium Müzik Oynatıcı" : type === "RETRO_CASSETTE" ? "Retro Kaset Çalar" : type === "SPOTIFY_CLASSIC" ? "Classic Spotify Player" : type === "VINYL_RETRO" ? "Retro Plak Çalar" : type === "GLASS_AUDIO" ? "Modern Cam Efekti" : type === "NEON_CYBERPUNK" ? "Neon Cyberpunk Player" : type === "MUSIC_PODCAST" ? "Müzik & Podcast Çalar" : type === "PORTFOLIO_GALLERY" ? "Portfolyo & Galeri" : type === "COUNTDOWN_LAUNCH" ? "Geri Sayım & Lansman" : type === "PREMIUM_VIDEO" ? "Premium Video" : "");
